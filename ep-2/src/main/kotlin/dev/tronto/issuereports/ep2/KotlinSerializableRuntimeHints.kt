...
class KotlinSerializableRuntimeHints : RuntimeHintsRegistrar {
    ...
    override fun registerHints(hints: RuntimeHints, classLoader: ClassLoader?) {
        ...
        val targetClasses = serializableProvider.findCandidateComponents("dev.tronto.issuereports.ep2.register")
        targetClasses.forEach {
            val javaClass = Class.forName(it.beanClassName)
            val companionObject = javaClass.kotlin.companionObject?.java ?: kotlin.run {
                println("$javaClass does not have companion object")
                return@forEach
            }
            val companionObjectField = try {
            ...
            val method = companionObject.getMethod(
                "serializer",
